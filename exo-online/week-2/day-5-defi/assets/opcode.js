...
function bytesToAsm(bytes) {
  ...
  for(var b=0;b<bytes.length;b++) {
    ...
    if(byte >= 0x4c && byte <= 0x4e){ 
    	var pushBytes = ""; 
    	var pushData = parseInt(codeops[byte].substring(11)); 
    	for(var i=pushData; i>0; i--){ 
    		pushBytes = bytes[b+i] + pushBytes; 
    	} 
	    commands.push(bytesToHexString(bytes.slice(b+1+pushData, b+1+pushData+pushBytes))); 
	    b+=(1+pushData+pushBytes); 
	    continue; 
    }
